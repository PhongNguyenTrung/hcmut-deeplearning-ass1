...
```bash
# Train 2 image models (ResNet-50, ViT-B/16) on CIFAR-100
python scripts/train_image.py

# Train a single image model
python scripts/train_image.py --model resnet50   # or vit_b16

# Train 2 text models (GRU, DistilBERT) on 20 Newsgroups
python scripts/train_text.py
python scripts/train_text.py --model distilbert  # or gru

# CLIP zero-shot image-text retrieval on Flickr30k test set
...
- `GRUClassifier(vocab_size, embed_dim, hidden_dim, num_layers, num_classes, dropout)`
- `get_distilbert(num_classes)` — HuggingFace sequence classification
...
`get_cifar100_loaders` uses `img_size=32` (CIFAR native). For ViT-B/16 (requires 224×224), `scripts/train_image.py` creates separate loaders with resize transforms automatically.
